...
def _build_prompt(category, merchant, trigger, customer=None, conv_turns=None):
    ...
    peer_ctr = ps.get("avg_ctr", 0)
    ctr_status = f"{'ABOVE' if my_ctr > peer_ctr else 'BELOW'} peer average ({peer_ctr:.3f})"

    lines = ["=== CONTEXT FACTS (MANDATORY GROUNDING) ==="]
    lines.append(f"MERCHANT: {idn.get('name','')} (Owner: {idn.get('owner_first_name','')})")
    lines.append(f"CATEGORY: {category.get('slug','')} | Voice: {v.get('tone','')} | Code-mix: {v.get('code_mix','')}")
    lines.append(f"PERFORMANCE: CTR={my_ctr:.3f} ({ctr_status}) | 7d Views: {d7.get('views_pct',0)*100:+.1f}% | 7d Calls: {d7.get('calls_pct',0)*100:+.1f}%")
    lines.append(f"SIGNALS: {', '.join(sigs) if sigs else 'None'}")
    lines.append(f"ACTIVE OFFERS: {', '.join([o['title'] for o in offers if o.get('status')=='active']) or 'None'}")
    
    if digest_ref:
        lines.append(f"DIGEST ITEM: '{digest_ref.get('title','')}' Source: {digest_ref.get('source','')} Date: {digest_ref.get('date','')}")
    
    lines.append(f"TRIGGER: kind='{trigger.get('kind','')}' urgency={trigger.get('urgency','2')}")
    if customer:
        c_idn = customer.get("identity",{})
        lines.append(f"CUSTOMER: {c_idn.get('name','')} | Prefers: {customer.get('preferences',{}).get('preferred_slots','any')}")
    
    if conv_turns:
        lines.append("\n=== CONVERSATION SO FAR ===")
        for t in conv_turns[-4:]:
            lines.append(f"[{t['from'].upper()}]: {t['body']}")

    lines.append("\n=== TASK ===\nCompose for trigger kind. Use real numbers from above. No URLs. Output ONLY valid JSON.")
    return "\n".join(lines)
